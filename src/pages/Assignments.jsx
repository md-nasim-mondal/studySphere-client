  ...
  useEffect(() => {
    const getData = async () => {
      const { data } = await axios(
        ...
        }/all-assignments?page=${currentPage}&size=${itemsPerPage}&filter=${filter}&sort=${sort}&search=${search}`
      );
      setAssignments(data);
    };
    getData();
  ...
  useEffect(() => {
    const getCount = async () => {
      const { data } = await axios(
        `${
          import.meta.env.VITE_API_URL
        }/assignments-count?filter=${filter}&search=${search}`
      );
      setCount(data.count);
    };
    getCount();
          ...
          <div className='grid grid-cols-1 md:grid-cols-2  lg:grid-cols-3 gap-6 py-12 md:py-20'>
            {assignments?.map((assignment) => (
              <Assignment
                ...
                setAssignments={setAssignments}></Assignment>
            ))}
          </div>
        </div>
        {/* Pagination Section */}
